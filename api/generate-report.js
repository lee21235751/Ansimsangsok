...
이 숫자들은 정확한 세액이 아니라 "구간 추정"임을 리포트에 명시하되, 일반론이 아니라 이 숫자를 직접 인용해 계산 과정을 보여주는 방식으로 작성하세요.`;
  ...
  const conditionalInstructions = [
    ...
    hasUnequalIntent ? '특정 자녀에게 더 남기고 싶은 의향이 있으므로, simulation의 분배 비교 4단(my_wish→if_no_action→gap→after_defense)을 이 리포트의 심장으로 삼아 가장 공들여 쓰세요. if_no_action에는 "지금 그대로면 다른 자녀가 법적으로 돌려달라고 청구할 수 있는 금액은 약 OO원으로 추정됩니다"를 추정 금액을 직접 인용해 쓰고, after_defense와 defense_options에는 "약 X억 → 약 Y억"처럼 방어 후 금액이 줄어드는 수치 변화를 반드시 넣으세요. 유류분 반환은 원물반환이 원칙이나 2026년 개정으로 가액(현금)반환 원칙으로 전환되는 추세이므로(시행시점 확인 필요), 현금·보험으로 정산 재원을 마련해두면 부동산·사업체를 지키기 유리하다는 점을 방어 논리로 쓰되 "이미 현금으로만 반환된다"고 단정하지는 마세요.' : '특정 자녀에게 더 주려는 의향이 뚜렷하지 않으므로, simulation의 분배 비교에서 분배 자체의 다툼 소지는 낮다고 솔직히 안심시키되, gap과 if_no_action은 분쟁이 아니라 실제 위험(외국국적 자녀 서류 지연으로 균등 분배조차 수개월 묶임, 공유 부동산 교착, 디지털자산 접근 불가 등)으로 잡고 after_defense에는 "수개월 지연 → 수주"처럼 기간이 줄어드는 변화를 쓰세요.',
...

{
  ...
  "simulation": {
    ...
    "after_defense": "[분배 비교 4단] 아래 방어수단을 적용했을 때 분배가 어떻게 의도에 가까워지는지. 반드시 '약 X억 → 약 Y억' 또는 '수개월 지연 → 수주'처럼 수치·기간이 바뀌는 형태로. 유류분 반환은 원물반환이 원칙이나 2026년 개정으로 가액(현금)반환 원칙으로 전환되는 추세이므로(시행시점 확인 필요), 현금·보험으로 정산 재원을 미리 마련하면 부동산·사업체를 지키기 유리하다는 점을 설명하되 "이미 현금으로만 반환된다"고 단정하지 마세요.",
