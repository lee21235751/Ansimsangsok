...
function buildSimulation(deep, freeAns) {
  ...
  return {
    ...
    giftToHeir, giftAmountEok, giftAmountRange: deep.priorGiftAmount || null, priorGiftTiming: deep.priorGiftTiming || null,
...
const LEGAL_FACTS = {
  ...
  caregiving: '부양·간병 기여를 나중에 법적으로 인정받으려면 지금부터 증거를 남겨야 합니다. 통장 이체 내역, 병원·요양비 영수증, 간병 기록, 함께 산 기간을 보여주는 주민등록 등본 등을 모아두세요. 부모를 장기간 부양하거나 간병한 대가로 받은 재산은 보상적 증여로 평가되어 일정한 경우 유류분 반환 대상에서 제외될 수 있으므로, 이런 기록이 더욱 중요해졌습니다.',
...
function getContext(a, types, deep, sim) {
  ...
  if (types.some(t=>t.includes('유류분')) || types.some(t=>t.includes('형제자매')) || ['parents','spouse_only','siblings'].includes(sim && sim.inheritanceTier)) {
    lines.push('유류분 권리자: 형제자매는 2024년 4월 헌법재판소 위헌 결정으로 이미 유류분 권리가 폐지됨(자녀·배우자·부모만 유류분 보유, 직계비속·배우자는 법정상속분의 1/2, 직계존속은 1/3). 2026년 1월 1일부터 시행된 구하라법(상속권 상실 선고 제도, 민법 제1004조의2)으로 패륜상속인에 대한 상속권 상실선고 제도가 신설되었고, 부양·간병 등 기여에 대한 보상적 증여는 유류분 반환 대상에서 제외될 수 있도록 명문화됨.');
    lines.push('주의: 유언대용신탁은 과거 유류분 회피 수단으로 알려졌으나, 최근 판례는 신탁재산도 유류분 산정 기초재산에 포함시키는 추세이므로 만능 해법이 아님. 신탁만 믿고 다른 준비를 소홀히 하면 안 됨.');
    lines.push('합법적으로 유류분 영향을 줄이는 실무적 방법 3가지: ① 특정 자녀의 부양·간병·재산형성 기여를 구체적으로 입증할 자료(통장 이체내역, 간병기록, 진단서 등)를 미리 확보해 기여분으로 인정받는 방법(2026 개정으로 보상적 증여는 유류분에서 제외될 수 있음) ② 유류분반환청구권의 소멸시효(침해를 안 날로부터 1년, 상속개시일로부터 10년)를 정확히 이해하고 시기를 설계하는 방법 ③ 유언장에 증여·유증의 경위와 이유를 구체적으로 명시해 다른 상속인의 이해를 구하고 분쟁 가능성 자체를 낮추는 방법.');
  }
