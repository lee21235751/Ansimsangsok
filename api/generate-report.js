...
function getContext(a, types, deep, sim) {
  ...
  if (a.conflict==='없음') {
    lines.push('현재 갈등이 없더라도, 국내 상속 관련 소송 건수는 이혼소송의 약 2배이며 그중 다수가 소액(1억원 이하) 분쟁이라는 통계가 있음. 사이가 좋은 가족도 막상 상속이 개시되면 입장이 달라질 수 있으므로 미리 정리해두는 것이 안전함.');
  }
...
이 숫자들은 정확한 세액이 아니라 "구간 추정"임을 리포트에 명시하되, 일반론이 아니라 이 숫자를 직접 인용해 계산 과정을 보여주는 방식으로 작성하세요.`;
  ...
  const hasCaregive    = sim && sim.caregivingContribution && sim.caregivingContribution !== '없음 또는 미상';
  ...
  const conditionalInstructions = [
    ...
    hasUnequalIntent ? '특정 자녀에게 더 남기고 싶은 의향이 있으므로, 유류분 추정액 숫자를 직접 인용해 "다른 자녀가 청구 가능한 금액은 약 OO원으로 추정되며, 이를 줄이는 방법으로는 ① 사전 정산 ② 기여분 증빙 확보(2026 개정으로 기여 보상 증여는 유류분에서 제외됨) ③ 유언장에 증여 경위 명시" 식으로 구체적으로 작성하세요.' : null,
    hasBusiness ? '사업체·지분이 있으므로 가업상속공제 요건(피상속인 경영기간, 상속인 가업 종사 등)을 입력된 지분율 기준으로 적용 가능성을 구체적으로 언급하세요.' : null,
    hasCaregive ? '기여(부양·간병) 사실이 있으므로 기여분 주장의 근거가 될 수 있는 자료(통장 이체내역, 진단서, 간병 영수증 등)를 구체적으로 설명하세요.' : null,
    hasOverseas ? '해외자산이 있으므로: 한국 거주자는 전세계 자산이 한국 상속세 과세대상에 합산된다는 점, 해당국 절차가 별도로 필요하다는 점을 설명하세요. 영주권자는 한국 국적 유지 시 한국 민법이 그대로 적용됩니다. 해외 증권계좌가 있다면 가족관계증명서 영문공증·아포스티유가 필요합니다.' : null,
    hasForeignKids ? '외국 국적 자녀가 있으므로: 한국 민법상 동일한 상속권이 있음을 설명하되, 인감증명서 대체(현지 공증+아포스티유+번역공증), 재외공관 인증만으로는 등기소가 거부하는 경우, 생전 위임장을 미리 준비해두는 방법을 구체적으로 안내하세요.' : null,
    hasPredecease ? '먼저 사망한 자녀가 있으므로 대습상속(손주가 상속인이 되는 경우) 구조, 손주의 유류분 권리, 미성년 손주의 특별대리인 선임 필요성을 설명하세요.' : null,
  ].filter(Boolean).join('\n');
...
${isParentPerspective ? '[중요] 이 사용자는 부모님(또는 배우자)의 상속을 자녀/가족 입장에서 준비하고 있습니다. 리포트 전체에서 3인칭으로 일관되게 작성하세요.\n' : ''}
[진단]
...

{
  ...
  "sections": {
    ...
    "simulation": {
      ...
      "forced_share_check": "[가장 중요한 섹션] 사용자가 원하는 분배를 실행했을 때 다른 상속인이 유류분반환청구로 법적으로 가져갈 수 있는 정확한 금액을 구체 숫자로 제시하고, 그 금액을 줄이거나 방어할 수 있는 실무적 방법을 단계별로 제시",
      "tax_note": "과세대상 재산 구간과 대략적 공제 적용 가능성만 한두 문장으로 간단히 언급"
    },` : ''}
    "persons": {
      "title": "확인할 사람 관계",
  ...
  "legal_notice": "이 리포트는 입력하신 정보를 바탕으로 한 일반적 상속 정보 제공 목적이며, 개별 법적·세무적 판단을 대체하지 않습니다. 유류분 시뮬레이션 수치는 추정치이며, 실제 상속세액은 공제·공시지가 변동 등으로 달라질 수 있으므로 세무사 상담을 받으세요. 2026년 최신 법령(상속권 상실선고 제도, 기여분 인정 확대, 형제자매 유류분 폐지, 우회상속 규제, 자녀공제 5억원)을 기준으로 작성되었습니다.",
  ...
  const callOnce = async (extraInstruction) => {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 10000,
        system: '안심상속 유료 상세리포트 작성 시스템. 이 서비스의 목적은 "사용자가 원하는 자녀(가족)에게 더 많이 남길 수 있도록, 유류분 분쟁을 합법적으로 최소화하는 상속 설계를 돕는 것"임. 법률·세무 조언 금지(단, 구간 추정 계산 및 일반적 법령·판례 정보 제공은 허용). JSON만 반환. 코드블록 없이. 각 섹션 항목은 지정된 개수만큼만 작성하고 절대 초과하지 말 것. 전체 응답이 5000 토큰을 넘지 않도록 각 항목을 간결하게 작성할 것.',
