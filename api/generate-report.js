...
function buildSimulation(deep) {
  ...
  /* 데이터 정합성 점검: 부동산이 없다고 했는데(부동산 시가 입력 자체가 없음) 전체 재산이
     금융+해외 자산으로 설명되는 최대치를 크게 초과하면, 입력 어딘가가 빠졌다는 신호.
     예) 무료설문 "부동산 없음" + 전체재산 "100억 이상"인데 금융자산 상한은 12억 → 차액 설명 불가.
     이 경우 리포트가 추정 총액을 그대로 단정하지 않고 "재산 구성을 다시 확인하라"고 안내하도록 플래그 전달. */
  const finEok = finMidpoint[deep.financialAssets] ?? 0;
  const explainableMax = (realEstateEok || 0) + (finEok || 0) + (overseasEok || 0);
  const assetMismatch = totalEok != null && realEstateEok == null
    && totalEok > explainableMax + 5; /* 5억 이상 차이나면 설명 불가로 간주 */
