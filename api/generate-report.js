...
function getContext(a, types, deep, sim) {
  ...
  if (deep && deep.testatorHealth === '인지 능력 저하가 진행 중(치매 진단 등)') {
    lines.push('[핵심] 유언장을 작성하는 사람의 의사능력(인지능력)이 저하된 상태라면, 추후 다른 상속인이 "유언 당시 정상적인 판단이 불가능했다"는 이유로 유언무효확인소송을 제기할 수 있음 — 실제로 유언 효력 분쟁에서 가장 흔한 사유 중 하나임. 법원은 보통 진단서·인지기능검사 결과(MMSE 등), 유언 작성 시점과 진단 시점의 선후관계, 유언 내용의 합리성(특정인에게 유독 유리한지) 등을 종합적으로 살펴봄. 실무에서 의사능력 분쟁 가능성을 낮추기 위해 일반적으로 활용되는 방법으로는 ① 공정증서에 의한 유언(공증인과 증인 2인이 직접 확인하는 방식이라 자필증서보다 분쟁 시 효력을 인정받기 유리함) ② 유언 작성과 가까운 시점에 의사의 진단서나 소견서를 함께 받아 보관 ③ 가능하다면 작성 과정을 영상으로 남기는 방법 등이 있음. 다만 인지능력 저하의 정도가 이미 상당히 진행된 경우라면 유언 자체의 법적 효력이 처음부터 의심받을 수 있으므로, 이 부분은 일반적 안내로 마무리하고 변호사·의료진과의 상담을 통한 개별 판단이 필요하다는 점을 분명히 안내하세요.');
  } else if (deep && deep.testatorHealth === '가벼운 건망증 등 경미한 변화가 있음') {
    lines.push('참고: 가벼운 인지 변화가 있는 단계에서는 통상 유언 능력 자체는 인정되는 경우가 많지만, 시간이 지날수록 의사능력 입증이 더 어려워질 수 있음. 작성 시점을 너무 늦추지 않는 것이 일반적으로 권장되며, 위와 같이 공정증서 유언 방식을 활용하면 분쟁 가능성을 낮추는 데 도움이 된다는 점을 간단히 안내하세요.');
  }
