...
이 숫자들은 정확한 세액이 아니라 "구간 추정"임을 리포트에 명시하되, 일반론이 아니라 이 숫자를 직접 인용해 계산 과정을 보여주는 방식으로 작성하세요.`;
  ...
  const conditionalInstructions = [
    ...
    hasUnequalIntent ? '특정 자녀에게 더 남기고 싶은 의향이 있으므로, 추정 금액을 직접 인용해 "다른 자녀가 법적으로 돌려달라고 청구할 수 있는 금액은 약 OO원으로 추정됩니다"라고 쓰고, 이를 줄이는 방법 3가지를 번호 붙여 설명하세요.' : null,
...
${conditionalInstructions}`;
  ...
  const systemMsg = '안심상속 유료 상세리포트 작성 시스템. 목적: "사용자가 원하는 자녀(가족)에게 더 많이 남길 수 있도록 유류분 분쟁을 합법적으로 최소화하는 상속 설계를 돕는 것". 법률·세무 조언 금지(구간 추정 계산 및 일반적 법령·판례 정보 제공은 허용). JSON만 반환. 코드블록 없이. 배열 항목은 반드시 별도 문자열로. 절대 금지 사항: ①"무료진단", "심화설문", "딥서베이", "리포트", "시스템", "모순", "데이터 불일치" 등 내부 시스템 용어를 고객에게 보이는 텍스트에 절대 사용하지 말 것. ②"(들)", "(또는 ~)" 같은 프롬프트 작성 스타일 표현 사용 금지. ③고객이 시스템 내부 구조를 알 수 있는 어떤 표현도 금지. 고객 입장에서 자연스럽고 전문적인 어조로만 작성할 것.';
