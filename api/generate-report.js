...
function matchTypes(a) {
  ...
  if (/빚|채무|보증|소송|손해배상|부담|모르겠음/.test(debt) && !/확인된 빚이나 소송은 없습니다|^없음$/.test(debt)) {
    add('T10',40);
    add('T11',25);
    add('T20',15);
  }
...
function buildSimulation(deep, freeAns) {
  ...
  /* 무료진단은 특정 가족에게 준 재산만 확인하므로, 수증자가 공동상속인인지와
     그 증여가 특별수익인지 입력만으로 단정하지 않는다. */
  const hasPriorGift = !!deep.priorGiftAmount;
  ...
  return {
    ...
    hasPriorGift, giftAmountEok, giftAmountRange: deep.priorGiftAmount || null, priorGiftTiming: deep.priorGiftTiming || null,
    ...
    insuranceBeneficiary: deep.insuranceBeneficiary || null,
...
const LEGAL_FACTS = {
  gift: '생전 증여가 유류분에 어떻게 반영되는지는 누구에게 어떤 취지로 증여했는지부터 구분해야 합니다. 공동상속인이 받은 증여가 상속분의 선급인 특별수익에 해당하면, 상속개시 1년보다 오래된 증여라도 단순히 시간이 지났다는 이유만으로 유류분 산정에서 빠지는 것은 아닙니다. 다만 모든 생전 증여가 특별수익인 것은 아니며, 상당한 기간의 동거·간호 등 특별한 부양이나 재산 유지·증가에 대한 보상으로 이루어진 증여·유증은 그 기여에 상응하는 범위에서 특별수익에서 제외됩니다. 공동상속인이 아닌 사람에 대한 증여에는 민법 제1114조의 상속개시 전 1년 기준이 원칙적으로 적용되고, 당사자 쌍방이 유류분권리자에게 손해를 가할 것을 알고 증여한 경우에는 그 이전의 것도 산입될 수 있습니다. 따라서 증여 시기만 보지 말고 수증자, 증여 목적, 금액, 부양·기여 사정을 함께 정리해야 합니다.',
  ...
  caregiving: '민법 제1008조 단서는 상당한 기간의 동거·간호 등으로 피상속인을 특별히 부양하거나 재산의 유지·증가에 특별히 기여한 데 대한 보상으로 이루어진 증여·유증은 그 기여에 상응하는 범위에서 특별수익에서 제외하도록 합니다. 이 규정은 2024년 4월 25일 이후 상속이 개시된 경우에도 적용됩니다. 통장 이체내역, 병원·요양비 영수증, 간병 기록, 동거 기간 자료, 증여 경위를 함께 남겨두는 것이 중요합니다.',
  valueReturn: '유류분 반환 방법은 2026년 3월 시행된 개정 민법으로 가액(현금)반환이 원칙으로 바뀌었습니다(시행일 이후 개시된 상속에 적용되며, 시행 전에 개시된 상속은 종전대로 원물반환이 원칙입니다). 어느 경우든 현금·보험으로 정산 재원을 미리 마련해두면 부동산·사업체 자체를 지키기에 유리합니다.',
  ...
  unrecognizedChild: '가족관계증명서에 올라 있지 않은 자녀(혼외자 등)가 있다면, 그 자녀가 친자로 인정(인지)되는 순간 다른 자녀와 똑같은 상속분을 갖습니다. 더 중요한 점은, 이미 상속 재산을 다 나눈 뒤에 그 자녀가 인정되더라도 자기 몫에 해당하는 금액을 다른 상속인들에게 청구할 수 있다는 것입니다(민법 제1014조) — 끝난 줄 알았던 상속이 다시 흔들릴 수 있습니다. 다만 인지청구 자체는 부모의 사망을 안 날로부터 2년 이내에 해야 하고, 그 후 금액을 청구하는 것도 자신의 몫이 침해된 것을 안 날로부터 3년·상속 분할이 있은 날로부터 10년이라는 기한이 있습니다. 생전에 미리 정리하거나 유언에 입장을 명시해두는 것이 안전합니다.',
  ...
  parentNeglect: '어린 시절 본인에 대한 부양 의무를 중대하게 위반했거나 학대한 부모님이 있다면, 2026년 시행된 구하라법(상속권 상실 선고 제도)으로 그 부모님의 상속권을 법원에 제한해달라고 청구할 수 있습니다. 다만 자동으로 배제되는 것이 아니라 반드시 법원에 따로 청구해야 하고, 상속인이 되었음을 안 날로부터 6개월 이내에 청구해야 하며, 양육비 미지급 내역·주민등록 분리 기간·학교나 복지기관 기록 등이 증거가 됩니다. 본인이 생전에 공정증서 유언으로 그 의사를 미리 남겨두는 방법도 있습니다.',
...
/* 보험금은 수익자 지정 상태에 따라 상속재산 여부·유류분 산입이 갈리므로, 입력값별로 맞춤 안내한다(검수 전이므로 단정은 피하고 확인을 권하는 톤). */
function buildInsuranceNote(sim){
  const v = sim && sim.insuranceBeneficiary;
  if(!v) return null;
  if(/없다|해당 없음|확인 안 해봤다/.test(v)){
    if(/확인 안 해봤다/.test(v)){
      return { title:'사망보험금 수익자, 한 번은 확인해두세요', content:'사망보험금은 수익자를 누구로 지정해두었는지에 따라 상속재산이 되기도 하고, 지정된 사람의 고유재산이 되기도 합니다. 이 차이는 누가 얼마를 받는지뿐 아니라 유류분 계산에도 영향을 줄 수 있어, 가입한 보험들의 수익자가 현재 누구로 돼 있는지 한 번 확인해두는 것이 좋습니다. 원하는 분배가 있다면 수익자 지정을 그에 맞게 점검해두세요.' };
    }
    return null;
  }
  if(/받을 사람이 본인으로/.test(v)){
    return { title:'보험 수익자가 본인이면 그 보험금은 상속재산이 됩니다', content:'사망보험금의 수익자를 본인(피보험자 자신)으로 지정해 둔 경우, 그 보험금은 상속재산에 포함되어 다른 상속재산과 똑같이 분할·유류분·상속세의 대상이 됩니다. 특정 사람에게 보험금을 남기려는 의도였다면 수익자를 그 사람으로 직접 지정하는 것과 효과가 다르므로, 지금 수익자 지정이 본인 의도와 맞는지 확인해보는 것이 좋습니다.' };
  }
  if(/특정한 한 사람/.test(v)){
    return { title:'특정인을 보험 수익자로 지정해도 유류분까지 완전히 피하긴 어렵습니다', content:'수익자를 특정한 한 사람으로 지정한 사망보험금은 원칙적으로 상속재산이 아니라 그 사람의 고유재산이 되어, 원하는 사람에게 자금을 남기는 수단으로 활용할 수 있습니다. 다만 그 수익자가 공동상속인(예: 특정 자녀)인 경우, 보험료를 피상속인이 부담했다면 그 보험금이 특별수익으로 보아 유류분 계산에 포함될 수 있다는 판례가 있어, 보험만으로 유류분을 완전히 피할 수 있다고 보기는 어렵습니다. 구체적인 적용은 변호사 상담에서 확인하시는 것이 안전합니다.' };
  }
  if(/여러 명|상속인 모두/.test(v)){
    return { title:'보험 수익자가 "상속인 모두"라면 각자의 고유재산이 됩니다', content:'수익자를 상속인 전원 또는 여러 명으로 지정한 사망보험금은 각 상속인의 고유재산으로, 원칙적으로 상속재산 분할 대상에서는 빠집니다. 다만 이 경우에도 누가 얼마를 받도록 비율을 정해두지 않으면 분쟁의 소지가 남을 수 있고, 상속세 계산에서는 보험금도 과세 대상이 되는 점은 함께 확인해두시는 것이 좋습니다.' };
  }
  return null;
}
...
function buildLegalShareText(sim){
  ...
  if(sim.spouseAlive && sim.childCount>0){
    return '배우자는 법정상속분으로 순재산의 약 '+pct(sim.spouseShareEok)+'%인 '+formatEok(sim.spouseShareEok)+'을, 자녀는 1명당 약 '+pct(sim.childShareEok)+'%인 '+formatEok(sim.childShareEok)+'을 받습니다(협의나 유언이 없을 때 적용되는 기본값). 누군가 이보다 적게 받으면 최소한 받아야 하는 몫인 유류분을 청구할 수 있는데, 그 금액은 배우자 '+formatEok(sim.spouseForcedShareEok)+', 자녀 1명당 '+formatEok(sim.childForcedShareEok)+'입니다(각자 법정상속분의 절반).';
  }
  if(!sim.spouseAlive && sim.childCount>0){
    return '자녀는 1명당 약 '+pct(sim.childShareEok)+'%인 '+formatEok(sim.childShareEok)+'을 법정상속분으로 받습니다(협의나 유언이 없을 때 기준). 유류분은 그 절반인 1명당 '+formatEok(sim.childForcedShareEok)+'입니다.';
  }
  return sim.shareText ? (sim.shareText+'. 형제자매에게는 유류분이 인정되지 않습니다.') : null;
}

function buildTaxNote(sim){
  if(!sim || sim.netEok==null) return null;
...
function buildDefenseOptions(sim){
  ...
  opts.push({ name:'생전 증여', cost:'낮음~중간',
    ...
    note:'생전 증여만으로 유류분을 피할 수 있다고 볼 수는 없습니다. 공동상속인이 받은 증여가 특별수익에 해당하면 오래전에 한 증여도 유류분 산정에 반영될 수 있고, 특별한 부양·기여에 대한 보상 부분은 다르게 취급될 수 있습니다.' });
...
function buildAfterDefense(sim){
  ...
  const hasBiz = sim.businessShare && sim.businessShare !== '없음 또는 미상';
  if(hasRE || hasBiz){
    return '현금·보험으로 정산 재원을 미리 마련해두면, 부동산이나 사업체를 팔지 않고도 다른 상속인의 유류분에 정산할 수 있어 원하는 분배에 더 가깝게 지킬 수 있습니다. 2026년 3월 시행된 개정 민법으로 유류분 반환이 가액(현금)반환 원칙으로 바뀌었으므로, 정산 재원을 확보해두는 것이 더욱 유효한 대비가 됩니다.';
  }
...
function buildBlindSpots(sim, answers, deep){
  ...
  if(sim.hasPriorGift)                                          push('사전증여는 시기보다 수증자와 증여 성격을 함께 봐야 합니다', LEGAL_FACTS.gift);
  ...
  const insNote = buildInsuranceNote(sim);
  if(insNote)                                                 push(insNote.title, insNote.content);
...
function hasDebtRisk(answers, deep, sim){
  const freeDebt = String((answers && answers.debt) || '');
  const deepDebt = String((deep && deep.debt) || '');
  const debtEok = sim && sim.debtEok;
  if (typeof debtEok === 'number' && debtEok > 0) return true;
  const riskText = freeDebt + ' ' + deepDebt;
  return /빚|채무|대출|카드|세금|보증|소송|손해배상|부담|모르겠음|미상|정확한 금액은 모르겠음/.test(riskText)
    && !/확인된 빚이나 소송은 없습니다/.test(freeDebt)
    && !/^없음$/.test(freeDebt.trim())
    && !/^없음$/.test(deepDebt.trim());
}

function buildDebtRiskSection(){
  return {
    title: '숨겨진 채무·소송 확인',
    content: '보이는 재산만으로 상속 상황을 판단하지 않는 것이 중요합니다. 아파트와 예금이 확인되더라도 대출, 카드채무, 세금, 보증채무처럼 가족이 모르고 있던 채무가 남아 있을 수 있습니다. 사망 당시 진행 중이던 소송이나 손해배상 문제가 있는지도 함께 확인할 필요가 있습니다. 상속재산에 대한 처분행위는 단순승인으로 평가될 수 있는 경우가 있으므로, 채무 규모가 불분명하다면 상속재산을 먼저 사용하거나 처분하기 전에 재산과 채무를 확인하는 것이 좋습니다.'
  };
}

function hasEstrangedCoheir(deep){
  const detail = deep && deep.conflictDetail;
  const text = Array.isArray(detail) ? detail.join(', ') : String(detail || '');
  return /연락이 끊긴 가족|연락.*끊|연락.*되지 않는|연락두절/.test(text);
}

function buildEstrangedCoheirSection(){
  return {
    title: '연락되지 않는 공동상속인 확인',
    content: '공동상속인 중 연락이 되지 않는 사람이 있으면 나머지 상속인들만으로 재산 정리를 마무리하기 어려울 수 있습니다. 현재 상속인 구성과 필요한 절차를 확인할 사항으로 별도 정리해 두세요.'
  };
}
...
function getContext(a, types, deep, sim) {
  ...
  if (wantsAvoidStepInheritance) {
    lines.push('[핵심] 본인이 먼저 사망하면 재산 일부가 배우자에게 가는데, 그 배우자가 나중에 사망하면 그 재산(원래 본인 재산이었던 부분 포함)은 배우자의 고유재산이 되어 배우자의 직계비속(배우자의 전혼 자녀 포함)에게 그대로 상속됨. 본인의 친자식이 아닌 배우자의 전혼 자녀에게 결국 자신의 재산이 흘러가는 것을 막을 방법이 없는가에 대해, 재산 규모와 무관하게 활용 가능한 방법들을 비용·난이도 순으로 설명하세요. ① 유언장에서 배우자 몫을 법정상속분이 아니라 유류분 수준(법정상속분의 1/2)까지로 정하고 나머지는 친자식에게 직접 분배하도록 지정하는 방법 — 배우자에게 가는 절대액이 줄어들면 나중에 의붓자식에게 흘러갈 수 있는 금액도 함께 줄어드는 구조. 별도 비용이 거의 들지 않음. ② 생전에 친자식에게 직접 사전증여하는 방법 — 배우자를 거치지 않고 바로 자녀에게 재산을 이전하는 단순한 방법(10년간 5천만원까지는 증여세 공제 적용). 소액부터 가능. ③ 종신보험 등에서 수익자를 친자식으로 직접 지정하는 방법 — 보험금은 원칙적으로 상속재산이 아니라 지정된 수익자의 고유재산이 되므로, 큰 목돈 없이도 활용 가능. 다만 공동상속인(자녀 등)을 수익자로 지정한 경우, 보험금이 고유재산이더라도 그 보험료를 피상속인이 납부했다면 특별수익으로 보아 유류분 산정의 기초재산에 포함될 수 있다는 하급심 판례가 있으므로, 보험만으로 유류분을 완전히 피할 수 있다고 단정하지는 말고 그 한계도 함께 설명하세요(상속인이 아닌 제3자를 수익자로 한 경우에는 상속개시 전 1년 이내에 지정·변경했거나 쌍방이 유류분 침해를 알고 한 경우에만 유류분에 산입된다는 점에서 차이가 있습니다). ④ 유언대용신탁 — 1차 수익자를 배우자로, 배우자 사망 후 잔여재산의 2차(후순위) 수익자를 친자식으로 지정하는 연속수익자 구조로, 일반 유언장으로는 구현할 수 없는 방법이지만 신탁 설계 비용과 최소 가입금액이 있어 재산 규모가 작다면 ①~③이 먼저 검토되는 경우가 많고, 여력이 될 때 추가로 고려되는 방법. 어떤 방법을 쓰든 배우자의 유류분(법정상속분의 1/2)은 완전히 배제할 수 없다는 공통 한계가 있다는 점도 함께 설명하세요.');
  }
  ...
  if (types.some(t=>t.includes('유류분')) || types.some(t=>t.includes('형제자매')) || ['parents','spouse_only','siblings'].includes(sim && sim.inheritanceTier)) {
    lines.push('유류분 권리자: 형제자매는 2024년 4월 헌법재판소 위헌 결정으로 이미 유류분 권리가 폐지됨(자녀·배우자·부모만 유류분 보유, 직계비속·배우자는 법정상속분의 1/2, 직계존속은 1/3). 상속권 상실 선고 제도는 2026년 1월 1일 시행되었고 2026년 3월 17일 개정으로 적용 범위가 보완됨. 법원이 상속권 상실을 선고하면 그 사람은 상속인 지위를 잃으며, 자동 배제가 아니라 법원의 선고가 필요함.');
    lines.push('주의: 유언대용신탁은 과거 유류분 회피 수단으로 알려졌으나, 최근 판례는 신탁재산도 유류분 산정 기초재산에 포함시키는 추세이므로 만능 해법이 아님. 신탁만 믿고 다른 준비를 소홀히 하면 안 됨.');
    lines.push('합법적으로 유류분 영향을 줄이는 실무적 방법 3가지: ① 특정 자녀의 부양·간병·재산형성 기여를 구체적으로 입증할 자료(통장 이체내역, 간병기록, 진단서 등)를 미리 확보해 기여분과 보상적 증여·유증 사정을 설명하는 방법(민법 제1008조 단서는 2024년 4월 25일 이후 개시된 상속에도 적용됨) ② 유류분반환청구권의 소멸시효(침해를 안 날로부터 1년, 상속개시일로부터 10년)를 정확히 이해하고 시기를 설계하는 방법 ③ 유언장에 증여·유증의 경위와 이유를 구체적으로 명시해 다른 상속인의 이해를 구하고 분쟁 가능성 자체를 낮추는 방법.');
  }
  ...
  if (a.gift==='일부 있음'||a.gift==='상당히 있음') {
    lines.push('핵심: 생전증여는 먼저 수증자가 실제 공동상속인인지, 그리고 증여가 상속분의 선급인 특별수익에 해당하는지 구분해야 함. 공동상속인의 특별수익이라면 오래전에 한 증여도 단순히 시간이 지났다는 이유만으로 유류분 산정에서 빠지지 않는 것이 원칙임. 모든 증여가 특별수익인 것은 아님. 특별한 부양·간병이나 재산 유지·증가에 대한 보상으로 한 증여·유증은 그 기여 범위에서 특별수익에서 제외됨. 공동상속인이 아닌 사람에 대한 증여에는 민법 제1114조의 1년 기준이 원칙적으로 적용되고, 쌍방이 유류분 침해를 알고 한 경우에는 더 이전 증여도 산입될 수 있음.');
  }
  ...
  if (hasDebtRisk(a, deep, sim)) {
    lines.push('채무 위험 신호가 있으므로 보이는 재산만 기준으로 판단하지 말고, 대출·카드대금·미납금·세금·보증채무·진행 중인 소송·손해배상 청구·부동산 담보·개인 간 채무를 함께 확인할 항목으로 정리하세요. 상속포기·한정승인은 원칙적으로 상속개시가 있음을 안 날부터 3개월 안에 검토할 수 있다는 일반 정보만 안내하고, 특정 제도를 선택하라고 단정하지 마세요.');
  }
  if (hasEstrangedCoheir(deep)) {
    lines.push('공동상속인 중 연락이 되지 않는 사람이 있으면 나머지 상속인들만으로 재산 정리를 마무리하기 어려울 수 있음. 구체적인 법률절차를 확정적으로 지시하지 말고, 상속인 구성과 필요한 절차를 확인할 사항으로 별도 정리하도록 안내하세요.');
  }
  return lines.join('\n');
}
...
- 사전증여 입력 여부: ${sim.hasPriorGift ? '예 (시기: '+(sim.priorGiftTiming||'미상')+', 수증자의 공동상속인 여부와 특별수익 해당 여부는 현재 입력만으로 확정하지 않음)':'없음 또는 미상'}
- 사전증여 추정액: ${sim.giftAmountRange ? '"' + sim.giftAmountRange + '" 구간(사용자가 고른 금액 구간임. 본문에서 특정 금액으로 단정하지 말고 이 구간 표현 그대로 쓸 것)' : formatEok(sim.giftAmountEok)}
...
이 숫자들은 정확한 세액이 아니라 "구간 추정"임을 리포트에 명시하되, 일반론이 아니라 이 숫자를 직접 인용해 계산 과정을 보여주는 방식으로 작성하세요.`;
  ...
  const conditionalInstructions = [
    ...
    hasUnequalIntent ? '특정 자녀에게 더 남기고 싶은 의향이 있으므로, simulation의 분배 비교 4단(my_wish→if_no_action→gap→after_defense)을 이 리포트의 심장으로 삼아 가장 공들여 쓰세요. if_no_action에는 "지금 그대로면 다른 자녀가 법적으로 돌려달라고 청구할 수 있는 금액은 약 OO원으로 추정됩니다"를 추정 금액을 직접 인용해 쓰고, after_defense와 defense_options에는 "약 X억 → 약 Y억"처럼 방어 후 금액이 줄어드는 수치 변화를 반드시 넣으세요. 2026년 3월 시행된 개정 민법으로 유류분 반환이 가액(현금)반환 원칙으로 바뀌었으므로, 현금·보험으로 정산 재원을 마련해두면 부동산·사업체를 지키기 유리하다는 점을 방어 논리로 쓰세요.' : '특정 자녀에게 더 주려는 의향이 뚜렷하지 않으므로, simulation의 분배 비교에서 분배 자체의 다툼 소지는 낮다고 솔직히 안심시키되, gap과 if_no_action은 분쟁이 아니라 실제 위험(외국국적 자녀 서류 지연으로 균등 분배조차 수개월 묶임, 공유 부동산 교착, 디지털자산 접근 불가 등)으로 잡고 after_defense에는 "수개월 지연 → 수주"처럼 기간이 줄어드는 변화를 쓰세요.',
    hasBusiness ? '사업체·지분이 있으므로: 가업상속공제라는 세금 경감 제도가 있다고 쉽게 언급하되, 요건은 "자녀가 몇 년 일했는지"가 아니라 피상속인(부모)이 장기간(10년 이상) 경영한 기업인지 등 여러 조건이 핵심이므로, 구체 요건·연수는 단정해 쓰지 말고 "세무사 상담이 꼭 필요하다"로 넘기세요.' : null,
...
${conditionalInstructions}`;
  ...
  const systemMsg = '안심상속 유료 상세리포트 작성 시스템. 목적: "사용자가 원하는 자녀(가족)에게 더 많이 남길 수 있도록 유류분 분쟁을 합법적으로 최소화하는 상속 설계를 돕는 것". 절세는 부차정보이고, "내 뜻대로 분배가 지켜지는가"와 유류분이 항상 메인. 법률·세무 조언 금지(구간 추정 계산 및 일반적 법령·판례 정보 제공은 허용). 이 리포트는 특정인의 사건을 대리·처리하는 법률상담이 아니라 일반 정보 제공·교육 자료입니다. 법률판단은 하지 않되 정보는 자신 있게 설명하고, 결과를 보장하지 않되 고객이 무엇을 확인하고 준비해야 하는지 명확하게 알려줄 것. 모든 문장에 "경우에 따라", "전문가 상담"을 반복하지 말고, 실제 개별 판단·신고·소송 대응 등 전문 판단이 필요한 지점에서만 전문가 확인을 안내할 것. JSON만 반환. 코드블록 없이. 배열 항목은 반드시 별도 문자열로. [방어수단 원칙] 비용이 낮은 순서로 권할 것: 유언 → 사전증여 → 보험 → 신탁. 신탁은 최소가입금액이 커서 순재산이 작은 사용자에겐 권하지 말 것. [2026년 개정 법령 — 정확 반영] ①형제자매 유류분은 폐지됨(2024.4.25 위헌, 즉시 효력 상실), 형제가 상속인이어도 유류분 청구 불가. ②유류분 반환 방법: 2026.3.17 시행된 개정 민법으로 가액(금전)반환이 원칙으로 변경됨. 시행일 이후 개시된 상속(피상속인 사망)에는 가액반환이 원칙으로 적용되고, 시행 전에 개시된 상속에는 종전대로 원물반환이 원칙임. 따라서 시행일 이후 사망 사건은 "가액(현금)반환이 원칙"으로 서술하되, 사용자의 상속개시 시점을 알 수 없으므로 단정보다는 개정 시행 사실을 안내하는 톤으로 쓸 것. 현금·보험으로 정산 재원을 미리 마련해두면 부동산·사업체를 지키기 유리하다는 방어 논리는 유효함. ③생전증여: 모든 상속인 증여를 자동 합산으로 단정하지 말 것. 공동상속인의 증여가 상속분의 선급인 특별수익이면 오래됐다는 이유만으로 유류분 산정에서 제외되지 않음. 보상적 증여·유증은 기여 범위에서 특별수익에서 제외되고, 민법 제1008조 단서는 2024-04-25 이후 상속개시에도 적용됨. 공동상속인이 아닌 사람 증여에는 민법 제1114조 1년 기준이 원칙이며, 쌍방이 유류분권리자에게 손해를 가할 것을 알고 증여한 경우에는 그 이전의 것도 산입될 수 있음. ④상속권 상실 선고 제도는 2026.1.1 시행되었고, 2026.3.17 개정으로 적용 범위가 보완됨. 법원이 상속권 상실을 선고하면 그 사람은 상속인 지위를 잃으며, 자동 배제가 아니라 법원의 선고가 필요함. [상속세 — 정확 반영] 자녀공제 1인 5억 확대안과 최고세율 50%→40% 인하안은 2026년 현재 국회 미통과/부결 상태이므로, 통과된 것처럼 쓰지 말 것. 현행 기준(자녀공제 1인 5천만원, 일괄공제 5억, 배우자공제 최소 5억~최대 30억, 최고세율 50%)으로 안내하되, 배우자+자녀 동시 상속 시 통상 약 10억(일괄 5억+배우자 최소 5억) 공제로 단순 추정할 수 있음. 가상자산(암호화폐) 양도소득 과세는 2027년으로 연기됨. 절대 금지 사항: ①"무료진단", "심화설문", "딥서베이", "리포트", "시스템", "모순", "데이터 불일치" 등 내부 시스템 용어를 고객에게 보이는 텍스트에 절대 사용하지 말 것. ②"(들)", "(또는 ~)" 같은 프롬프트 작성 스타일 표현 사용 금지. ③"가족과 함께", "자녀에게 보여주세요" 등 본인 외 공유를 전제하는 표현 금지(본인만 조용히 확인하는 서비스). ④고객이 시스템 내부 구조를 알 수 있는 어떤 표현도 금지. [법률 정확성 — 반드시 준수] ⓐ유류분 맥락에서 생전증여를 설명할 때 모든 상속인 증여를 자동 합산으로 단정하지 말 것. 공동상속인이 받은 증여가 특별수익(상속분의 선급)에 해당하면 오래됐다는 이유만으로 빠지지 않지만, 모든 증여가 특별수익은 아니며 보상적 증여·유증은 기여 범위에서 특별수익에서 제외됨. 공동상속인이 아닌 사람 증여에는 민법 제1114조 1년 기준이 원칙임. ⓑ상속세 배우자공제를 언급할 땐 반드시 "최소 5억~최대 30억"으로 쓰고 "최소 5억"만 단독으로 쓰지 말 것. ⓒ가업상속공제 요건을 "자녀가 몇 년 일했다"로 설명하지 말 것. ⓓ배우자가 한국 국적이면 해외 영주권이 있어도 한국에서 인감증명으로 상속 처리가 가능하므로, 배우자에게 현지 공증·아포스티유가 필요하다고 쓰지 말 것(그 절차는 외국 단일국적 자녀에게만 해당). ⓔ해외금융계좌 신고제도(연중 잔액 5억 초과 시 신고)는 해당 해외자산이 5억 원 미만이면 언급하지 말 것. ⓕ보험 등 방어수단의 금액을 "20~30억" 같은 구체 숫자로 단정하지 말 것 — 필요한 정산 재원은 분배 설계에 따라 달라지므로 "정산에 필요한 만큼"으로 설명할 것. ⓕ-2 보험 수익자 지정 효과(상속재산인지 고유재산인지, 특별수익으로 유류분에 산입되는지)는 blind_spots에 코드로 정확히 안내되므로, 본문에서 보험을 언급할 때 "수익자 지정만으로 유류분을 피할 수 있다"는 식으로 단정하지 말 것. 특히 자녀 등 공동상속인을 수익자로 한 경우 특별수익으로 유류분에 산입될 수 있다는 한계를 무시하지 말 것. [수치 정확성 — 반드시 준수] ⓖ위에 제공된 추정액 숫자(전체 재산·순재산·법정상속분·유류분 등)는 그대로 인용만 하고 절대 새로 계산하지 말 것. ⓗ과세대상·과세표준 금액을 본문 prose에 직접 숫자로 쓰지 말 것(세무 수치는 tax_note 한 곳에만 둔다). ⓘ어떤 금액도 위에 제공된 전체 재산 추정액을 초과해 쓰지 말 것. 고객 입장에서 자연스럽고 전문적인 어조로만 작성할 것.';
...

{
  ...
  "simulation": {
    ...
    "after_defense": "[분배 비교 4단] 아래 방어수단을 적용했을 때 분배가 어떻게 의도에 가까워지는지. 반드시 '약 X억 → 약 Y억' 또는 '수개월 지연 → 수주'처럼 수치·기간이 바뀌는 형태로. 2026년 3월 시행된 개정 민법으로 유류분 반환이 가액(현금)반환 원칙으로 바뀌었으므로, 현금·보험으로 정산 재원을 미리 마련하면 부동산·사업체를 지키기 유리하다는 점을 설명하세요.",
...
규칙: JSON만 반환. 코드블록 없이. 각 항목은 2~3문장으로 간결하게 작성하세요. 불필요하게 길게 쓰지 마세요.`;
  ...
  const RISK_PATTERNS = [
    { re: /((유류분[\s\S]{0,60}10\s*년)|(10\s*년[\s\S]{0,60}유류분))[\s\S]{0,30}(빠진다|빠집니다|빠지지\s*않|제외|소멸|지나면\s*(빠|없|청구|반환\s*못)|청구\s*(못|불)|받을\s*수\s*없)/, exempt: /(상속세|증여세|다릅니다|다르다|관한\s*규정|특별수익|상속분의\s*선급|민법\s*제1114조)/, name: "유류분 맥락 '10년' 오결론(제3자 증여는 1년)" },
  ...
  /* 입력 맥락 기반 추가 검증(정적 패턴으로 못 잡는 케이스). sim을 참조해 해당 사용자에게
     틀리거나 무관한 수치가 새어나오면 잡는다. 걸리면 아래 callPartChecked가 1회 재생성한다. */
  const ctxChecks = (obj) => {
    const s = JSON.stringify(obj);
    const out = [];
    const overseasEok = sim.hasOverseasAsset ? (sim.overseasEok || 0) : 0;
    if (overseasEok < 5 && /해외[\s\S]{0,30}(금융)?계좌[\s\S]{0,40}5\s*억[\s\S]{0,25}신고/.test(s)) {
      out.push('해외금융계좌 5억 신고를 비해당 케이스에 언급(ⓔ 위반)');
    }
    const cap = (sim.totalEok || 0) * 1.5 + 5;
    const nums = (s.match(/\d+(?:\.\d+)?\s*억/g) || []).map(x => parseFloat(x));
    if ((sim.totalEok || 0) > 5 && nums.some(n => n > cap && n > 50)) {
      out.push('전체 재산 추정치를 크게 초과하는 금액 등장(재계산 환각 의심)');
    }
    const autoGiftAll = /(상속인|자녀|공동상속인)[\s\S]{0,40}증여[\s\S]{0,60}(시기와\s*무관|기간\s*제한\s*없이)[\s\S]{0,30}(전부|모두|항상)[\s\S]{0,20}(합산|산입)/.test(s);
    const hasSpecialBenefitCondition = /(특별수익|상속분의\s*선급)/.test(s);
    if (autoGiftAll && !hasSpecialBenefitCondition) {
      out.push('특별수익 판단 없이 자동 전부 합산 단정');
    }
    if (/(보상적\s*증여|부양·간병[\s\S]{0,20}보상|보상으로[\s\S]{0,20}증여)[\s\S]{0,80}2026[.\-\s년]*1[.\-\s월]*1/.test(s)) {
      out.push('보상적 증여 2026-01-01 적용시점 오기재');
    }
    return out;
  };
  ...
  const callPartChecked = async (prompt, partName) => {
    ...
    if (issues.length) {
      console.warn(`[generate-report] ${partName} 법률검증 경고: ${issues.join(', ')} — 1회 재생성`);
      try {
        const retry = await callPart(prompt, partName);
        if (checkLegal(retry).length < issues.length) obj = retry;
      } catch (e) { console.error(`[generate-report] ${partName} 재생성 실패: ${e.message}`); }
      issues = checkLegal(obj);
      if (issues.length) console.error(`[generate-report] ${partName} 재생성 후에도 위험표현 잔존: ${issues.join(', ')} — 수동 검토 필요`);
    }
    return obj;
  };
  ...
  const __report = {
    ...
    legal_notice: '이 리포트는 입력하신 정보를 바탕으로 일반적인 상속 정보를 정리해 제공하는 자료이며, 특정 사건에 대한 법률 자문이나 법률 상담이 아닙니다. 변호사·세무사의 개별 상담을 대체하지 않습니다. 유류분 시뮬레이션 수치는 추정치이며, 실제 상속세액은 공제·공시지가 변동 등으로 달라질 수 있으므로 세무사 상담을 받으세요. 형제자매 유류분 폐지(2024.4.25 위헌), 상속권 상실 선고 제도(2026.1.1 시행 및 2026.3.17 개정), 2026.3.17 개정 민법의 가액반환 원칙, 특별한 부양·기여에 대한 보상적 증여·유증의 특별수익 제외, 민법 제1008조 단서의 2024.4.25 이후 상속개시 적용 등 최근 개정 사항을 반영했습니다. 상속세 자녀공제 확대(1인 5억) 및 최고세율 인하 등은 2026년 현재 국회를 통과하지 않아 반영하지 않았으며, 현행 공제(일괄공제 5억, 배우자공제 최소 5억~최대 30억) 기준으로 안내합니다. 세법은 개정 논의가 진행 중이므로 신고 시점에 최신 내용을 확인하세요.',
  ...
  try {
    ...
    if (hasDebtRisk(answers, deepAnswers, sim)) {
      if (!__report.sections.assets) __report.sections.assets = { title: '재산과 채무', checklist: [] };
      __report.sections.assets.checklist = appendUnique(__report.sections.assets.checklist, [
        '금융기관 대출·카드대금·미납금',
        '다른 사람의 채무에 대한 보증 여부',
        '국세·지방세 체납 또는 미납',
        '진행 중인 민사소송·손해배상 청구',
        ...
        '연락되지 않는 공동상속인',
        '생전에 특정 상속인이 받은 증여',
        '장기간 부양·간병 또는 재산 유지에 특별히 기여한 사정',
        '상속재산을 먼저 사용하거나 처분하기 전에 확인할 항목'
      ]);
      const debtItem = buildDebtRiskSection();
      const blindItems = (__report.sections.blind_spots && Array.isArray(__report.sections.blind_spots.items)) ? __report.sections.blind_spots.items : [];
      if (!blindItems.some(x => x && x.title === debtItem.title)) blindItems.unshift(debtItem);
      __report.sections.blind_spots = { title: '놓치기 쉬운 항목', items: blindItems.slice(0,8) };
    }
    if (hasEstrangedCoheir(deepAnswers)) {
      const coheirItem = buildEstrangedCoheirSection();
      if (!__report.sections.persons) __report.sections.persons = { title: '확인할 사람 관계', checklist: [] };
      __report.sections.persons.checklist = appendUnique(__report.sections.persons.checklist, [coheirItem.content]);
      const blindItems = (__report.sections.blind_spots && Array.isArray(__report.sections.blind_spots.items)) ? __report.sections.blind_spots.items : [];
      if (!blindItems.some(x => x && x.title === coheirItem.title)) blindItems.push(coheirItem);
      __report.sections.blind_spots = { title: '놓치기 쉬운 항목', items: blindItems.slice(0,8) };
    }
